...
$produk = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM produk WHERE id='{$_GET['id']}'"));
?>
    ...
    <div class="card" style="max-width: 600px; margin: 0 auto;">
        <form action="/indomaret/process/products_process.php" method="POST">
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" list="voucher_list" name="id_voucher" value="<?= htmlspecialchars($produk['id_voucher']) ?>" placeholder="Pilih voucher (opsional)" />
                <datalist id="voucher_list">
                    <?php
                    $query = mysqli_query($conn, "SELECT * FROM voucher");
                    while ($voucher = mysqli_fetch_assoc($query)) {
                        $selected = ($voucher['id'] == $produk['id_voucher']) ? "selected" : "";
                    ?>
                        <option value="<?= $voucher['id'] ?>" <?= $selected ?>>
            ...
            <div class="form-group">
                ...
                <input type="number" class="form-control" name="harga_satuan" value="<?= $produk['harga_satuan'] ?>" min="0" required />
            </div>
